...
This document describes all configuration options and settings available in the Action Result Entities custom integration.
...
```yaml
service: action_result.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: action_result.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
...
```yaml
blueprint:
  name: Action Result Entities Alert
...
- [GitHub Issues](https://github.com/jpawlowski/hass.action_result/issues) - Report problems
