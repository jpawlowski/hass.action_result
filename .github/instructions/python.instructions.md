...
- Files: `snake_case.py`
- Classes: `PascalCase` prefixed with `ActionResultEntities`
...
- Inherit from both platform entity and `ActionResultEntitiesEntity` (order matters)
...
from custom_components.action_result.sensor import async_setup_entry
...
**Config entry data:** `entry_data: ActionResultEntitiesData = hass.data[DOMAIN][entry.entry_id]`
